import { AggregatedCandle } from '@/lib/dataManager/aggregator';
import { useState, useEffect, useRef } from 'react';

const STORAGE_KEY_PREFIX = 'finnsays_chart_history_';

export function useChartHistory(symbol: string, initialData: AggregatedCandle[]) {
    const [history, setHistory] = useState<AggregatedCandle[]>(initialData);
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (stored) {
                const parsed = JSON.parse(stored) as AggregatedCandle[];
